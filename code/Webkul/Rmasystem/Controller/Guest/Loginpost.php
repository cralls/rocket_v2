...
class Loginpost extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $order = $this->_objectManager->create(\Magento\Sales\Model\Order::class)->loadByIncrementId($post["order_id"]);
        $email = $order->getCustomerEmail();
        if ($email == $post["email"] && $order->getCustomerGroupId() == 0) {
            $this->messageManager->addSuccess(
                __('Login Successfully.')
            );
            $this->_objectManager->create(\Magento\Framework\Session\SessionManager::class)->setGuestData($post);
            $this->coreRegistry->register('guest_data', $post);
            return $resultRedirect->setPath("*/guest/rmalist");
        } else {
            $this->_objectManager->create(\Magento\Framework\Session\SessionManager::class)->unsetGuestData();
