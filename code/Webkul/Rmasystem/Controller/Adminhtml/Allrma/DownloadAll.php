...
class DownloadAll extends Action
{
    ...
    public function execute()
    {
        ...
        $allFiles = array_diff(\Magento\Framework\Filesystem\Driver\File::readDirectory($filePath), ['..', '.']);
        ...
        $response = $om->get(\Magento\Framework\App\ResponseInterface::class);
        $response->setHeader('Content-Type', 'application/zip', true);
        $response->setHeader('Content-disposition: attachment; filename='.$zipname);
        $response->setHeader('Content-Length: ' . \Magento\Framework\File\Size::getFileSizeInMb($zipname));
        \Magento\Framework\Filesystem\DriverInterface::fileRead($zipname);
        return true;
    }
