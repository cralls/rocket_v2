...
class Saverma extends \Webkul\Rmasystem\Controller\FrontController
{
    ...
    public function execute()
    {
        ...
        $msg = "Invalid requested item qty(s).";
        $error = false;
        ...
        $post['image'] = $this->helper->getSerializeData($imageArray);
        ...
        if ($this->getRequest()->getFiles('related_images')) {
            try {
                ...
                $model->setImage($this->helper->getSerializeData($imageArray));
                $model->save();
            } catch (\Exception $e) {
                $this->messageManager->addError($e->getMessage());
            }
