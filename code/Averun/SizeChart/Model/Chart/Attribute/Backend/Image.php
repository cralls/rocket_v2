...
namespace Averun\SizeChart\Model\Chart\Attribute\Backend;
...
class Image extends AbstractBackend
{
    ...
    public function beforeSave($object) //@codingStandardsIgnoreLine
    {
        ...
        $file = reset($file) ?: [];
        if (!isset($file['file'])) {
            throw new LocalizedException(__('Image does not contain field \'file\''));
        }
        // Add file related data to object
        $object->setImage($file['file']);
    ...
    public function afterSave($object) //@codingStandardsIgnoreLine
    {
        ...
        if ($object->getFileExists() || $object->getSkipSaveImage()) {
            return $this;
        }

        // Delete old file if new one has changed
        if ($object->getOrigData('image') && $object->getImage() != $object->getOrigData('image')) {
            $this->fileProcessor->delete($this->getFileSubDir($object), $object->getOrigData('image'));
        }

        if ($object->getImage()) {
            if (!$this->fileProcessor->saveFileFromTmp($object->getImage(), $this->getFileSubDir($object))) {
    ...
    {
        return self::FILES_SUBDIR . '/' . $object->getIdentifier();
    }
    ...
    public function beforeDelete($object) //@codingStandardsIgnoreLine
    {
        ...
        if ($object->getImage()) {
            $this->fileProcessor->delete($this->getFileSubDir($object), $object->getImage());
        }
        return $this;
    }
    ...
    {
        if (!$object->getData('file_info') && $object->getImage()) {
            $fileInfoObject = new DataObject();
            $fileInfo = $this->fileProcessor->getFileInfo($object->getImage(), $this->getFileSubDir($object));
            if ($fileInfo) {
