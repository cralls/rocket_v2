 ...
 * @custom     VNS 2026-06-24: Only pass jersey_numbers when parent product show_numbers is enabled.
 */
...
class UpdateItemOptions extends \Magento\Checkout\Controller\Cart\UpdateItemOptions
{
    ...
    protected function addChildProduct($params)
    {
        ...
        $parentProduct = $this->helperClass->returnProductFactory()
            ->create()
            ->setStoreId($storeId)
            ->load($params['product']);
        $showJerseyNumbers = $this->isShowNumbersEnabled($parentProduct);
        foreach ($params['config_table_qty'] as $id => $qty) {
            if (isset($qty)) {
                ...
                if ($showJerseyNumbers) {
                    $data['jersey_numbers'] = isset($params['jersey_numbers'][$id])
                        ? trim((string)$params['jersey_numbers'][$id])
                        : '';
                }
    ...
    /**
     * Return true only when the parent configurable product has show_numbers enabled.
     *
     * @param object|null $product
     * @return bool
     */
    protected function isShowNumbersEnabled($product)
    {
