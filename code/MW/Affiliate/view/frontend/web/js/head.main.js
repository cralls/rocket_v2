define([
    ...
    window.MW.Affiliate.Report.Dashboard = Class.create();
    MW.Affiliate.Report.Dashboard.prototype = {
        initialize: function(params){
            ...
            jQuery("#aff_report_from").calendar({
                showsTime: true,
                dateFormat: 'mm/dd/yy',
                timeFormat: 'HH:mm:ss'
            });
            jQuery("#aff_report_to").calendar({
                showsTime: true,
                dateFormat: 'mm/dd/yy',
                timeFormat: 'HH:mm:ss'
            });
        ...
        onClickRefresh: function(event, element){
            ...
            jQuery.ajax({
                ...
                type : "post",
                dataType:"json",
                data: {ajax: true, report_range: $("aff_report_range").value, from: $("aff_report_from").value,  to: $("aff_report_to").value, type: 'dashboard'},
                success : function (transport){
                    if(transport){
                        self.data = transport.report;
                        self.buildChart(parseInt($("aff_report_range").value));
                        self.buildPieChartCommissionByMembers(transport.report_commission_by_members);
                        self.buildPieChartCommissionbyPrograms(transport.report_commission_by_programs);
                        self.fillDataStats(transport.statistics);
                        self.fillDataInvitation(transport.invitation_report);
                        self.statistics = transport.statistics;
                    }else{
                        /** Draw empty grah */
                    }
                }
        ...
        onChangeRange: function(event, element){
            ...
            jQuery.ajax({
                ...
                type : "post",
                dataType:"json",
                data : {ajax: true, report_range: element.value, type: 'dashboard'},
                success : function (transport){
                    if(transport){
                        self.data = transport.report;
        ...
        fillDataInvitation : function(data){
            $("aff_total_click").innerHTML = data.click;
            $("aff_total_register").innerHTML = data.register;
            $("aff_total_purchased").innerHTML = data.purchase;
            $("aff_total_subscribe").innerHTML = data.subscribe;
        },
